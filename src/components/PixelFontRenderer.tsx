...
const FONT_CATEGORIES = [
  ...
  },
  {
    label: "System Fonts",
    fonts: [
      { name: "serif", label: "System Serif", weight: "bold", url: null as string | null },
      { name: "sans-serif", label: "System Sans", weight: "bold", url: null as string | null },
      { name: "monospace", label: "Monospace", weight: "bold", url: null as string | null },
    ],
  },
];
...
const ALL_FONTS: Font[] = FONT_CATEGORIES.flatMap((c) => c.fonts);
...
function pixelateChar({ char, fontFamily, fontWeight, gridSize, offsetX = 0, offsetY = 0, scale = 1.0, threshold = 128 }: {
  char: string; fontFamily: string; fontWeight: string; gridSize: number;
  offsetX?: number; offsetY?: number; scale?: number; threshold?: number;
}) {
...
function renderSmooth({ char, fontFamily, fontWeight, size, offsetX = 0, offsetY = 0, scale = 1.0 }: {
  char: string; fontFamily: string; fontWeight: string; size: number;
...
const PixelGrid = ({ pixels, cellSize, gap, showGrid, color = "#0a0a0a" }: {
  pixels: number[][]; cellSize: number; gap: number; showGrid: boolean; color?: string;
}) => {
...
export default function PixelFontRenderer() {
  ...
  const [selectedFont, setSelectedFont] = useState<Font>(ALL_FONTS[0]);
  const [fontsLoaded, setFontsLoaded] = useState<Set<string>>(new Set());
  const [pixels, setPixels] = useState<number[][]>([]);
  const [smoothImg, setSmoothImg] = useState<string | null>(null);
  ...
  const addCustomFont = async () => {
    ...
    const newFont: Font = { name: customFontName.trim(), label: `✦ ${customFontName.trim()}`, weight: "400", url };
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "28px 20px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "300px 1fr", gap: 24, alignItems: "start" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <div style={{ background: "white", border: "2px solid #1a1a1a", padding: 20 }}>
              <div style={{ fontSize: 10, textTransform: "uppercase", letterSpacing: "0.1em", color: "#888", marginBottom: 14 }}>
                All Blackletter &amp; Gothic Fonts · &ldquo;{char}&rdquo; at 16px
              </div>
            ...
            <div style={{ background: "#f0ede8", padding: 16, fontSize: 11, lineHeight: 1.7, color: "#666" }}>
              ...
              into the &ldquo;Add any Google Font&rdquo; field — the CSS URL is auto-generated. Try these gothic/medieval options:{" "}
...
const codeStyle: CSSProperties = { background: "#e0ddd8", padding: "1px 5px", borderRadius: 2 };
